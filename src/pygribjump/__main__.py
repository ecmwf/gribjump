...
import pygribjump._internal as _internal
...
DEPENDENCY_ORDER = ["eckit", "eccodes", "metkit", "fdb5", "gribjump"]
OPTIONAL_DEPENDENCIES = ["eccodes", "fdb5"]


def main():
    parser = argparse.ArgumentParser(description="pygribjump command line interface")
    parser.add_argument(
        "--print-home",
        action="store_true",
        help="Print the home directory of the gribjump library",
    )
    parser.add_argument(
        "--print-home-deps",
        action="store_true",
        help="Print the home directories of all pygribjump dependencies",
    )
    parser.add_argument(
        "-v",
        "--verbose",
    ...
    if args.print_home:
        dependency_path = findlibs.find("gribjump")
        if dependency_path is None:
            logging.error("gribjump library not found by findlibs")
            sys.exit(1)
