...
/// Single source of truth for the remote gribjump wire protocol.
///
/// Every message on the wire has exactly one encoder and one decoder, and they
/// live side by side here. Both the client (RemoteGribJump) and the server
/// (GribJumpUser / Request) drive their I/O exclusively through this class, so
/// the two sides cannot drift apart: a change to a byte layout is a single,
/// reviewable edit to a matched encode/decode pair.
///
...
/// remoteProtocolVersion (Protocol.h) and regenerate the golden hashes in
