...
pub enum Error {
    ...
    /// Generic error from the gribjump C++ library.
    #[error("gribjump error: {0}")]
    Gribjump(String),
...
mod tests {
    ...
    fn convert_message(msg: &str) -> Error {
        ...
        if let Some(rest) = msg.strip_prefix("ECKIT_SERIOUS_BUG: ") {
            Error::SeriousBug(rest.to_string())
        } else if let Some(rest) = msg.strip_prefix("ECKIT_USER_ERROR: ") {
    ...
    fn test_no_prefix_falls_through() {
        ...
        assert!(matches!(err, Error::Gribjump(msg) if msg == "plain error message"));
    }

    #[test]
    fn test_std_exception_no_prefix() {
        let err = convert_message("std::runtime_error message");
        assert!(matches!(err, Error::Gribjump(msg) if msg == "std::runtime_error message"));
    }
}
