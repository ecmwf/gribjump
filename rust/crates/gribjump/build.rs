//! Build script for gribjump crate.
//!
...
//!   `gribjump-sys`'s build script re-publishes each dependency's
//!   lib dir via `cargo:system_*_lib` metadata keys, and we emit an
...
fn main() {
    ...
    // When gribjump-sys is in system mode, it re-publishes each
    ...
    for key in [
        "DEP_GRIBJUMP_SYS_SYSTEM_GRIBJUMP_LIB",
        "DEP_GRIBJUMP_SYS_SYSTEM_FDB5_LIB",
        "DEP_GRIBJUMP_SYS_SYSTEM_ECKIT_LIB",
        "DEP_GRIBJUMP_SYS_SYSTEM_METKIT_LIB",
        "DEP_GRIBJUMP_SYS_SYSTEM_ECCODES_LIB",
    ] {
